...
public class BuyingACar {
    public static int[] nbMonths(int startPriceOld, int startPriceNew, int savingperMonth, double percentLossByMonth) {
        ...
        int money = 0;
        int counter = 1;
        while (oldPrice + money < newPrice) {
            if ((counter % 2 == 0)) {
                percent += 0.5;
            }
            oldPrice -= (((percent) / 100) * oldPrice);
            newPrice -= (((percent) / 100) * newPrice);
            money += savingperMonth;
            counter++;
        }
        return new int[]{counter - 1, (int) Math.round(money + oldPrice - newPrice)};
    }

    public static void main(String[] args) {
        int[] tab = nbMonths(2000, 8000, 1000, 1.5);
        for (Integer n : tab) {
...
or equal there is no saving to be made, no need to wait so he can at
